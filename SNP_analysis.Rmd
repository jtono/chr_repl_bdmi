...
Now plot with loess smoothing. Loess with span=0.2. Do for those mapped to paradoxus first.
```{r, echo=FALSE}
lo1 <- loess(ANcut4$reff1~ANcut4$pos, span=0.2)
lo2 <- loess(ANcut4$reff2~ANcut4$pos, span=0.2)
...
```
```{r, echo=FALSE}
#which positions in lowest 5%
ANlow <- ANcut4[which(fitsN<sort(fitsN)[length(fitsN)*0.05]),]$pos
...
\
Now plot with loess smoothing. Loess with span=0.2. Do for those mapped to cerevisiae.
```{r, echo=FALSE}
lo1 <- loess(AWcut4$altf1~AWcut4$pos, span=0.2)
lo2 <- loess(AWcut4$altf2~AWcut4$pos, span=0.2)
...
```{r, echo=FALSE}
#which positions in lowest 5%
AWlow <- AWcut4[which(fitsW<sort(fitsW)[length(fitsW)*0.05]),]$pos
