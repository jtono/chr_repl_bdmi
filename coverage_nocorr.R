...
#take a look at it
plot(cov15.cp$start.c, cov15.cp$depth.cer.c, type="l", ylim=c(0,600))
...
#no fully mismapped - all should map
quantile(cov15.cp.F$depth.cer.c)
q1 <- quantile(cov15.cp.F$depth.cer.c, 0.25)
q3 <- quantile(cov15.cp.F$depth.cer.c, 0.75)
iqr <- q3-q1
cov15.cp.F[which(cov15.cp.F$depth.cer.c<q1-1.5*iqr),]
#none
cov15.cp.F[which(cov15.cp.F$depth.cer.c>q3+1.5*iqr),]
#none

range(cov15.cp.F$depth.cer.p)
cov15.cp.F[which(cov15.cp.F$depth.cer.p>0),]
#YOL086C has one read mismapped
quantile(cov15.cp.F$depth.cer.p)
q1 <- quantile(cov15.cp.F$depth.cer.p, 0.25)
q3 <- quantile(cov15.cp.F$depth.cer.p, 0.75)
iqr <- q3-q1
cov15.cp.F[which(cov15.cp.F$depth.cer.p<q1-1.5*iqr),]
#none
cov15.cp.F[which(cov15.cp.F$depth.cer.p>q3+1.5*iqr),]
#YOL086C only one
######here#######
range(cov15.cp.F$depth.XVpar.c)
cov15.cp.F[which(cov15.cp.F$depth.XVpar.c>0),]
#YOL052C has 10 reads mismapped
quantile(cov15.cp.F$depth.cer.p)
q1 <- quantile(cov15.cp.F$depth.cer.p, 0.25)
q3 <- quantile(cov15.cp.F$depth.cer.p, 0.75)
iqr <- q3-q1
